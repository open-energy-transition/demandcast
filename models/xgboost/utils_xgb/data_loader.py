...
def load_annual_demand(folder_path: str) -> pandas.DataFrame:
    ...
    ----------
    folder_path : str
        Folder containing annual electricity demand per capita
        parquet files.
    ...
    pandas.DataFrame
        ...
        region_code, Annual electricity demand per capita (kWh).
    """
    files = [
        file_name
...
def load_gdp(folder_path: str) -> pandas.DataFrame:
    ...
    -------
    pandas.DataFrame
        Dataframe with columns: Time (UTC),
        GDP PPP per capita, region_code.
    """
    files = [
        file_name
        for file_name in os.listdir(folder_path)
...
def load_temperature(folder_path: str) -> pandas.DataFrame:
    ...
    for file_name in tqdm(files, desc="Loading temperature data"):
        ...
        # Handle both CODE_YEAR.parquet and
        # CODE_YEAR_SCENARIO.parquet patterns
        if len(parts) >= 2:
            region_code = (
                parts[0] + "_" + parts[1]
